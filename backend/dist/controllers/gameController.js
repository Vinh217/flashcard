...
const calculateScore = (isCorrect, answerTime) => {
    if (!isCorrect)
        return 0;
...
const getRandomVocabulary = async (limit) => {
    // Lấy tổng số từ vựng
...
const createQuizQuestion = async (correctVocab, allVocab) => {
...
const generateQuizQuestions = async (roomId) => {
    try {
...
// Khởi động game mới
const startNewGame = async (io, roomCode) => {
    try {
        // Tìm phòng theo mã
...
// Chuyển sang câu hỏi tiếp theo
const moveToNextQuestion = async (io, roomId) => {
    const gameState = gameStates.get(roomId);
    if (!gameState)
        return;
    // Reset trạng thái
...
const endGame = async (io, roomId) => {
    ...
    try {
        ...
        const results = roomUsers.map(ru => ({
            playerId: ru.user_id,
